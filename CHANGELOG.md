...
### Changed
- **Lyst 快照落後看門狗門檻 2→3 季：`LYST_STALE_QUARTERS`（D31，2026-07-03，擁有者開工 + AskUserQuestion 拍板「放寬門檻」）**：7/2 排程 patrol（每週一・四 cron）首次紅、追評到 6/16 起開著的 issue #122、且 #172 後每次失敗都對 Kai 發紅通知，本機確認唯一觸發 WARN＝「Lyst 快照落後 2 季（最新 2026-Q1）」——6/29（六月＝Q2，落後 1 季）還綠，7/1 進 Q3 落後變 2、剛跨過舊門檻。Lyst Index 季末後約一個月才發布（Q2 剛結束、很可能還沒發），且 D21 後 ingest 是對話端手動編 yaml、無自動管線可「斷」→ 這條紅擋在「暫時無法 action 的內容項」上，違反「警告必配修復」、紅燈衰退成噪音（同 D29 週挑）。改 `LYST_STALE_QUARTERS` 2→3：落後 2 季內歸 INFO（`--strict` 不再變紅、issue/Kai 通知停），落後 3 季才 WARN（那才是真的沒人 ingest）。同步 docstring（頂部 WARN 清單 + `check_lyst_staleness`）、`docs/decisions.md` D31。可逆（改回 2 即還原）。
