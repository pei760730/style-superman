...
### Fixed
- **自報 bot UA 被三個源的 WAF 擋、差點誤撤活源：`collect_raw_signals.UA` 換瀏覽器等級（2026-07-03，PR #177）**：`StyleSupermanBot/0.1` 自報身分 UA 被 bof 回 403、heddels / permanent-style 回**空 200**（WAF 對不明 bot 餵空——比 403 更毒，看起來像源死了）。同 URL 換瀏覽器 UA 實測三源全回完整 RSS（bof 120KB / heddels 18KB / permanent-style 394KB）——**死的是 UA，不是源**，三源曾被 liveness 誤判死源、差點被 D17 撤掉。這些是公開訂閱用 feed，一般 reader 等級 UA 屬正常消費。`repo_health._probe_rss` 共用 `c.UA` → 這一刀同治收集端與死源誤報主因（與 D32 重試互補：D32 治真瞬斷、本修治 UA 級封鎖）。同步 `docs/lessons.md`。可逆。
...
### Changed
- **Lyst 看門狗改「發布寬限」模型：`LYST_STALE_QUARTERS` → `LYST_PUBLISH_LAG_DAYS`（D31，2026-07-03，擁有者同日兩次拍板、發布寬限模型勝出）**：純日曆季差門檻讓每年 1/4/7/10 月頭六週固定假警報（behind 跳 2 但 Lyst 索引季末後 ~3-4 週才發布、根本無事可做）→ 7/2 patrol 紅的判紅主因。改為只有「上一季結束 + 45 天寬限已過」還沒 ingest 才 WARN：假警報根治、看門狗語意保留（真沒人 ingest 寬限一過就叫，不像門檻 3 季要鈍化到 ~9 個月）。第一版方案「門檻 2→3」（PR #175）未 merge 即關、由本模型取代。同步 `docs/decisions.md` D31。可逆。
