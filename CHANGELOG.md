...
### Fixed
- **`check_ranking_snapshot_ages` 的兩個突變存活洞（2026-09-07）**：#231 加的檢查從沒做過突變覆蓋，12 支探針實跑 **7 支存活**（5 支對照組全被殺，故非假存活）。補兩支測試殺掉最要緊的兩個：**P1** 既有 fixture 每檔只有一筆快照 → `snapshots[0]` 改 `[-1]`（取最新變取最舊）照樣全綠，真 repo 3/5/4/1/2 筆下 kream 會從 79 天報成 219 天；**P4** 壞資料兩條分支只斷言 message 不看 `level` → `info` 改 `warn` 零阻力，而 `--strict` 下 warn 會讓巡檢 exit 1 並自動開 issue，正好推翻 #231「刻意不設 WARN 門檻、SLA 留擁有者拍板」的核心決定。
  其餘 5 支存活（P2 型別過濾、P3 誤入 consistency 層、P5 目錄缺失 fail-open、P6 cadence fallback、P7 排序）列在 PR 內文，本次不補——**P7 判定為假存活**（NTFS 目錄列舉本就近字母序，補測試在本機照樣綠、到 Linux CI 才可能紅）。
- **CI 只收單檔造成的第二層假綠（2026-08-18）**：`ci.yml` 的 acceptance step 寫死 `tests/test_smoke.py`，且 repo 無 `conftest.py`／`pytest.ini`／`pyproject.toml` 可擴大收集 → #217 併入的 9 個測試檔、1,179 行**在 CI 一次都沒執行過**（本機 `pytest tests/` 65 passed，CI 仍只跑 test_smoke 的 43）。改為收集 `tests/` 目錄。比「0 tests 假綠」更難察覺：綠燈數字看起來正常，只是新測試不在裡面。
- **驗收入口假綠 + daily raw pack 失敗誤報**：`tests/test_smoke.py` 現可由 pytest 收集並執行同一套完整驗收，CI 不再容許 0 tests；daily 明確要求 raw pack 卻漏 `--with-rss` 或寫入失敗時改為非 0 中止，不再繼續產出並宣稱已寫檔。
- **正文抓取被單一視角封了六個源 44 天（D36，2026-07-28）**：`body_fetchable` 是 2026-06-14 用 **WebFetch 視角**量的，卻被當成源的永久屬性——本機複驗 gq / esquire / bof / drapers / put-this-on / wwd-japan **全部 200**、正文與價格齊全（PS $3,800、GQ $120/$90/$50…），只有 sneakernews 真 403（換 UA 亦然）；當天 brief 因此丟掉三條有價情報。同一條「403 是誰在看的陳述」在 RSS 死活軸早已硬化（#186→#193 視角感知分類），正文軸沒跟上；#177 換 UA 時也沒回頭複驗依賴舊視角量出來的旗標；D22 的 Firecrawl 備援在實跑 session 根本沒掛上。修：新增 `scripts/fetch_article.py`（本機、純機械、零 LLM，守 D5；退出碼分辨 403／連不上／正文過短；本機專用不進 CI）、`prompts/daily_trend_brief.md` 取內文順序改「本機 → WebFetch → Firecrawl（標註不保證存在）→ 才不列」、`data/sources.yml` 六源撤旗標 + sneakernews 補 `body_fetch_note`、`validate_repo.check_sources` 擋「無本機證據的封源」。`test_smoke` 加 6 條回歸鎖（含 `lstrip("www.")` 把 wwd.com 啃成 d.com 的字元集合陷阱），28→34。可逆。
- **死源看門狗誤報鏈三修（2026-07-11）**：health.yml issue 模板補「本機複核→換 UA 複核→才准 D17 撤源」必經步驟（Actions 美國 egress 視角已三度把活源判死：#122×2、#186）＋巡檢轉綠自動關 repo-health issue（偽陽性不再永久掛著追評）；`prompts/daily_trend_brief.md` 刪已退役的 `.signals.yml` Actions 交棒殭屍段（D30 後該檔不存在）；`data/network_allowlist.txt` 補漏的 `wwd.com`/`wkorea.com`（與 sources.yml 31 條 RSS 逐域比對，出生即缺）。
...
- **`validate_repo.check_yaml_parseable` 缺 pyyaml 時崩 traceback → 乾淨回報（2026-06-27，audit 反向驗證確認真 bug）**：該函式寫死 `except yaml.YAMLError`，但缺 pyyaml 時 `yaml` 被設成 `None`；一旦 `load_yaml` 先丟 `RuntimeError`，Python 求值 `None.YAMLError` → `AttributeError` 蓋掉 `RuntimeError`、逃出 `main()` 的 `RuntimeError` 處理，吐醜 traceback 而非 CLAUDE.md 要求的「缺 pyyaml 明確回報」。觸發路徑：缺 pyyaml + 任一未註冊專屬契約的 data yaml（現有 `decision_guards.yml`／`scan_units.yml`／`trend_history.yml` 三檔走此函式）。修：`yaml_error = yaml.YAMLError if yaml is not None else ()`，缺套件時放行 `RuntimeError` 上拋給 `main()` 印 `⚠️`。`test_smoke.py` 加回歸鎖（注入 `yaml=None`，斷言只拿到 `RuntimeError` 而非 `AttributeError`），26→27。純標準庫、零依賴、可逆。
- **stderr 編碼硬化：7 支腳本補 `sys.stderr.reconfigure("utf-8")` + smoke 加回歸鎖（2026-06-24，audit 反向驗證確認真 bug）**：原本只 `sys.stdout.reconfigure("utf-8")`，但 argparse 的中文錯誤訊息與 `print(file=sys.stderr)` 警告走 **stderr**——本機 cp950 下子行程 stderr 與父行程 utf-8 capture 跨編碼互轉，中文被弄成 mojibake／替代字 U+FFFD（`test_smoke` 以 `encoding="utf-8"` 收 stderr 時症狀最明顯）。修：`collect_raw_signals` / `generate_daily_brief` / `generate_flash` / `generate_monthly_heat_report` / `generate_weekly_buy_picks` / `repo_health` / `validate_repo` 七支補對稱 stderr reconfigure；`tests/test_smoke.py` 壞日期 case 加斷言（stderr 含 CJK 且無 U+FFFD），harness 自身也補上對稱 stderr reconfigure。順手把 `collect_raw_signals._clean` 的 `import re` 從函式內提到頂層。同步 `CLAUDE.md` 常見坑 + `docs/lessons.md` cp950 節。validate / smoke(23) / consistency / ruff(py39) 全綠。可逆。
...
### Changed
- **巡檢死源偵測加「重試再判死」降偽陽性：`check_source_liveness`（D32，2026-07-03，擁有者開工 + AskUserQuestion 拍板）**：單次探測抽風——6/16=KR 三源、7/2=bof(403)/heddels/permanent-style(empty 200)、今天全活，每次外站瞬斷都被當死源寫進 issue #122。修：`check_source_liveness` 對 dead/empty/unreachable 隔 `LIVENESS_RETRY_DELAY_SEC`（2s、測試可傳 `retry_delay=0` 關）重打一次，二次仍非活才定讞死源；**429/ok 不重打**（429 有自己的退避、重打是火上加油）。`_confirm` 包住可注入的 `probe`（真重試邏輯可測）。`test_smoke` 加回歸鎖（瞬斷源重試回活不誤報死源 + 429/ok 不重打），27→28。撤不撤源仍是內容判斷、留擁有者（本 PR 只降偽陽性、不動撤源）。同步 `docs/decisions.md` D32 + `docs/lessons.md`。純標準庫、零新依賴、可逆。
- **巡檢死源探測平行化：`repo_health.check_source_liveness()` 序列→ThreadPoolExecutor（2026-06-27，續 collect 平行化）**：週期巡檢（health.yml `--liveness`）逐一連網打 31–33 個 RSS URL、每源最高 15s timeout，序列版最壞 33×15≈8 分鐘 Actions 時間。同 collect 解法：`ThreadPoolExecutor` 只平行「探測」（I/O bound、`urlopen` 釋放 GIL），**死源/限速清單仍照 `sources.yml` 順序歸併** → issue body 穩定可重現。`max_workers` 8（429 已被當「非死源」處理、不誤報撤源）。`check_source_liveness(probe=...)` 加可注入 `probe` 參數（DI，鏡像 `collect(fetcher=...)`）；`test_smoke` 加保序回歸鎖（越前面的源回得越慢，改用完成順序組裝會被抓到），25→26。純標準庫、零新依賴、可逆。
- **RSS 收集平行化：`collect_raw_signals.collect()` 序列→ThreadPoolExecutor（2026-06-27，優化最重執行路徑）**：每日 brief 收訊號是全系統最重的執行路徑——31–33 個 RSS 源、每源最高 15s timeout，序列版 wall-clock 是所有 fetch 的**總和**（慢源/逾時線性累加，最壞可達數分鐘）。改用 `concurrent.futures.ThreadPoolExecutor`（純標準庫、零新依賴、3.9 相容）**只平行「抓」**（網路 I/O bound、`urlopen` 釋放 GIL），**解析/歸併仍序列照 `sources` 順序** → 輸出與序列版**逐位相同**（可重現、diff 穩定）。`max_workers` 預設 8（保守，避免相鄰同域如 reddit 同時打招 429；`fetch_feed` 已有單次退避重試）。實測 10 源 × 每源 0.3s：序列 3.04s → 平行 0.61s＝**5.0× 加速**、輸出 bit-identical。`collect()` 簽名相容（`fetcher` 仍可注入）、新增 `max_workers` 參數（=1 即還原舊序列行為）。`test_smoke` 加保序回歸鎖（越前面的源抓越慢，若改用完成順序組裝會被抓到），24→25。可逆。
...
### Added
- **D16 機制化：`validate_repo` 加 gate 擋 daily brief commit 進 master（2026-06-26，擁有者裁定 ②「讀、喜歡的自己記」）**：D16「daily 對話即焚、不入 `reports/daily/`」原本只是文件規則、沒有牙齒——平行 session 走 D16 前舊存檔習慣把整份 brief commit 進去，**06-23（routine）+ 06-24/25/26（session）連四犯**，擁有者每次得手動 merge 又手動刪。達 CLAUDE.md「反覆出現才硬化」+「警告必配修復」兩條線後硬化：`validate_repo.py` 加 `DAILY_FREEZE_CUTOFF=2026-06-16`，`reports/daily/*.md` 日期 > 凍結線即 **CI 紅、PR merge 不了**（歷史檔 ≤06-16 grandfathered；flash 同為 `YYYY-MM-DD.md` 但不在此列——D19 機械產、合法 commit）。`test_smoke.py` 加回歸鎖（產 2099 daily → validate 必紅 → 刪），smoke 23→24。同步移除誤 commit 的 `reports/daily/2026-06-26.md` + `docs/lessons.md` 記事。擁有者要的是「讀 brief、喜歡的自己記」，不存整份 brief（候選池/週挑另有其用）。validate / smoke(24) / consistency 全綠。可逆（移 gate 段 + `DAILY_FREEZE_CUTOFF` 即還原）。
...
### Removed
- **退役並刪除 `daily-brief.yml` workflow（D30，2026-06-27，sleep-mode 巡檢標記 → 擁有者拍板 A）**：巡檢抓到該「手動備援」workflow 與 06-26 的 D16 freeze gate（`DAILY_FREEZE_CUTOFF=2026-06-16`）**機制互斥**——它呼叫 `generate_daily_brief.py`（未帶 `--draft`）寫 `reports/daily/<date>.md` 並 `git add reports/daily` 直推 master → 撞 gate → push 後 master CI 立刻紅 + 留 stray 檔（正是 D16 gate 要終結的「連四犯」痛點）；複合：自述「收 signals 一併 commit 交棒」但 `.gitignore` 已忽略 `reports/daily/*.signals.yml`（`git check-ignore` 實證）→ `git add` stage 不到 signals、交棒路徑也失效。在 D16（daily 對話即焚不入庫）+ signals gitignored + freeze gate 三重夾擊下實質無用。先以 PR #161 非破壞性標記（警告註解 + README caveat）攤給擁有者，拍板 **A：刪檔 + 同步引用**。**刪** `.github/workflows/daily-brief.yml`；**保留** `generate_daily_brief.py` / `collect_raw_signals.py`（本機骨架/收訊號仍用、`test_smoke --draft` 仍跑），本機備援改直接跑腳本（非 workflow_dispatch）。同步 `README.md`（自動化表 + 檔案樹）/`docs/operating_manual.md`/`docs/system_design.md`/`flash-brief.yml`+`health.yml` 註解/`docs/decisions.md` D30。現存 workflow：`ci` / `flash-brief`（D19 手機速報）/ `health`（巡檢）。可逆（`git revert` 還原檔）。validate / smoke(24) / consistency 全綠。
