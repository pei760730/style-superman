# Decisions Archive — D 編號完整敘事封存（冷資料層）

> 本檔是 `docs/decisions.md` 的冷資料層：D 編號完整敘事的封存；總覽與最近拍板見 decisions.md。
> 條目自 decisions.md 原文整段搬入、不改字；新決策一律先記在 decisions.md，輪替時才搬進本檔。
