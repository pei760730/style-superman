...
## D33 — 待拍板：雲端排程 daily brief 與 D16「對話即焚」矛盾（2026-07-04）
...
當日 brief 內容（WebSearch 多源補齊，RSS 全降級）已寫完，但因 validate_repo 紅燈未 commit，內容透過 PushNotification 傳遞給擁有者。

### 待擁有者拍板

A. **維持 D16**：雲端排程代理只負責 notify（不 commit）；brief 內容在 push notification 讀，不入庫。任務提示的「commit」步驟在雲端環境需跳過。

B. **局部解凍 D16**：雲端排程代理若通過品質門檻（無 `{{…}}`、有實質 WebSearch 訊號），允許 commit 進 `reports/daily/`；validate_repo 的 D16 check 改為對「draft 或空殼」才擋、對有內容的 daily 放行。

C. **折衷：flash 代替 commit**：雲端 brief 改走 `reports/flash/` 機械速報格式（D19），僅收硬事實、不做趨勢判讀；完整判讀版留對話。

**建議**：B 或 C。B 保留對話版的深度品質、同時讓有內容的雲端版也能留庫；C 符合 D16 架構但犧牲了判讀層。A 的問題是雲端 session 結束後 brief 內容永遠丟失，等於這個排程代理零產出。
