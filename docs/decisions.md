...
## D31 — Lyst 看門狗改「發布寬限」模型（2026-07-03，7/2 patrol 紅 → 擁有者同日兩次拍板：先「門檻 2→3」（PR #175），後改本模型取代、#175 未 merge 即關）

### 背景

2026-07-02 排程「Repo Health Patrol」首次紅（6/29 還綠）、追評到 6/16 起開著的 issue #122（該 issue 是 6/16 死源事件開的，見 lessons 2026-06-16 節），且 #172 後 health job 一失敗就對 Kai 發紅通知——patrol cron 每週一・四（`0 1 * * 1,4`，health.yml 明寫「頻率刻意不設每日」），不修＝每週兩發紅。本機確認唯一觸發 WARN＝「Lyst 快照落後 2 季（最新 2026-Q1）」：`--strict` 把 WARN 當失敗 → job 紅。6/29 還在六月＝Q2、落後僅 1 季故綠；7/1 進 Q3 落後跳 2 → 紅——「同一份資料三天內綠轉紅」正是季界假警報的實演。根本節奏事實：**Lyst Index 在季結束後 ~3-4 週才發布**（Q2'25＝7/23 發），且 D21 後 ingest 是對話端手動編 yaml、無自動管線可「斷」。純日曆季差門檻（落後 >= 2 季就警）讓**每年 1/4/7/10 月的頭六週固定假警報**——behind 跳 2 但新索引根本還沒出、無事可做，違反「警告必配修復」。

### 兩案取捨（同日重新拍板）

- **門檻 2→3（PR #175，第一次拍板）**：假警報只是延後不是根治（每年跨季仍有「behind 達標但無新索引可 ingest」的窗），且把「真的沒人 ingest」的偵測鈍化到落後 3 季（~9 個月）才叫——看門狗語意打折。
- **發布寬限模型（PR #177，改採）**：`LYST_STALE_QUARTERS` 移除，改 `LYST_PUBLISH_LAG_DAYS = 45`——只有「上一季結束 + 45 天寬限已過、新一季索引照節奏該存在了」還沒 ingest 才 WARN。假警報根治（發布前永不叫）、看門狗語意保留（真沒人 ingest，寬限一過就叫、不用等 9 個月）。擁有者比較後選本案，#175 關閉未 merge（其 D31 初版文檔隨 PR 一起未入庫，本條為 D31 正式版）。

### 可逆 / guards

可逆（改回純季差門檻即還原）。無禁用識別字，不寫 `decision_guards`（純看門狗校準）。延續 D29（內容逾期不該讓看門狗長期紅）、D21（ingest 已對話化）、D7 反熵、CLAUDE.md「警告必配修復」。D32 文中「見 D31」即指本條。
...
### 追記（2026-07-03 同日）：背景段歸因已被同日實測部分推翻

上方背景把三組死源全歸「外站瞬斷」。同日 #177 實測訂正：**7/2 三源（bof/heddels/permanent-style）真因是自報 bot UA 被 WAF 擋**（換瀏覽器 UA 同 URL 即回滿 RSS；已修 `collect_raw_signals.UA`，詳見 lessons「自報 bot UA 被 WAF 擋」節）；6/16 KR 三源另有 Actions egress 視角因素（lessons 2026-06-16 節）。**重試機制保留**——防真瞬斷（DNS 抖、暫時 5xx）仍對症；但 UA 級封鎖要靠 UA 修，重試不是對症解。教訓：死源要分層歸因，先排除「是我們被擋」。
