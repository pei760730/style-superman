...
## D31 — Lyst 快照落後看門狗門檻 2→3 季（2026-07-03，開工巡檢 → 擁有者 AskUserQuestion 拍板「放寬門檻」）

### 背景

2026-07-02 排程「Repo Health Patrol」首次紅（6/29 還綠）、追評到 6/16 起開著的 issue #122（該 issue 是 6/16 死源事件開的，見 D32），且 #172 後 health job 一失敗就對 Kai 發紅通知——patrol cron 是**每週一・四**（`0 1 * * 1,4`，health.yml 明寫「頻率刻意不設每日」），不修＝**每週兩發紅**直到有人 ingest。本機跑 `repo_health.py` 確認唯一觸發 WARN＝**「Lyst 快照落後 2 季（最新 2026-Q1）」**：`--strict` 把 WARN 當失敗 → job 紅。6/29 還在六月＝Q2、落後僅 1 季故綠；7/1 進 Q3 → 落後 2 季，剛跨過舊門檻 `LYST_STALE_QUARTERS=2`——這也解釋了「同一份資料、三天內綠轉紅」。

問題本質同 D29（週挑）：**Lyst Index 季末後約一個月才發布**（Q2 剛結束、很可能還沒發布），且 **D21 後 ingest 已改成對話端手動編 yaml**（`ingest_ranking_snapshot.py` 已移除、無自動管線可「斷」）——code 原註解「落後兩季以上代表 ingest 斷了」是 D21 前的框架、已過時。這條紅擋在一個「工程端無法自動修、且當下可能根本無資料可 ingest」的內容項上，讓紅燈衰退成噪音，違反 CLAUDE.md「警告必配修復」與 D7 反熵。

### 反向驗證（撤源 vs 放寬 vs 降 INFO）

liveness 死源偵測（`--liveness`）**不進 `--strict`**、非本次判紅主因（另案 D32 處理其偽陽性）；判紅 100% 來自 Lyst-lag WARN。降 INFO（比照 D29 週挑）或放寬門檻皆可解。攤給擁有者，擁有者選**放寬門檻**（非降 INFO）：保留「真的沒人 ingest 很久」仍會 WARN 的看門狗語意，只是把「正常發布延遲 + 尚未 ingest」的容忍窗從 1 季拉到 2 季。

### 拍板：`LYST_STALE_QUARTERS` 2 → 3

- 落後 <= 2 季：INFO（`--strict` 不變紅、issue/Kai 通知停）。
- 落後 >= 3 季：WARN（此時已跨過正常發布延遲 + 一整季沒 ingest，才是真該提醒的斷更）。
- 同步更新 `scripts/repo_health.py` 常數註解、頂部模組 docstring WARN 清單、`check_lyst_staleness` docstring（移除過時的「落後兩季＝ingest 斷了」框架）、`CHANGELOG.md`。

### 可逆 / guards

可逆（`LYST_STALE_QUARTERS` 改回 2 即還原）。無禁用識別字，不寫 `decision_guards`（純門檻校準、非不可回頭的識別字禁令）。延續 D29（同類「內容逾期不該讓看門狗長期紅」）、D21（ingest 已對話化）、D7 反熵、CLAUDE.md「警告必配修復」。
