...
### 2026-07-03 · `--liveness` 死源偵測單探測抽風、誤報瞬斷源進 issue
- **發生什麼**：patrol（每週一・四排程）的死源清單三次三組、互不重疊、下次探測全恢復：06-16=KR 三源（unreachable）、07-02=bof(403)/heddels/permanent-style(empty 200)、07-03 本機實測=0 死源。外站瞬斷（暫時 403 / empty 200 / DNS 抖）被單次探測當成永久死源，開出／追評 issue #122（6/16 那組就是它開的 issue）。注意：liveness 步驟帶 `|| true`、死源不弄紅 job——7/2 job 紅是 strict/Lyst（D31），死源的破壞是**污染 issue 清單**。「死源」清單因此分不出真死 vs 瞬斷、失去可信度。
- **對策 / 硬化（2026-07-03，D32）**：`check_source_liveness` 對 dead/empty/unreachable 隔 `LIVENESS_RETRY_DELAY_SEC`（2s）重打一次，**二次仍非活**才定讞死源；429/ok 不重打（429 有自己的退避、重打招更多 429）。達 D7 門檻（同類抽風重複出現 + 對策穩定）才硬化，非單次事故加碼。撤不撤源仍是內容判斷、留擁有者（D17/D18），本次只降偵測偽陽性。教訓：**連網探測的「一次失敗」≠「壞了」——瞬斷要重試確認再定讞，否則看門狗自己製造噪音（呼應排程 workflow「跑了≠產出了」的同一族系）。**
- **⚠ 歸因訂正（同日）**：本節把 7/2 三源歸「外站瞬斷」是**誤診**——真因是自報 bot UA 被 WAF 擋（見下節），重試防不了那類封鎖；原文保留為歷史。

### 2026-07-03 · 自報 bot UA 被 WAF 擋，被誤讀成「瞬斷死源」（D32 同日訂正歸因）
- **發生什麼**：7/2 的三個「死源」（bof=403、heddels / permanent-style=空 200）當時被歸因「外站瞬斷」（見上節 D32）。同日稍後實測：同 URL 用 `StyleSupermanBot/0.1` 自報身分 UA 打＝403 / 空 200，換瀏覽器等級 UA 打＝三源全回完整 RSS（bof 120KB / heddels 18KB / permanent-style 394KB）。**死的是 UA，不是源、也不全是瞬斷**——WAF 對不明 bot 的封鎖是間歇性的（同日另一輪 liveness 探測全過），所以症狀長得像抽風瞬斷。三源差點被 D17 流程撤掉。
- **對策**：`collect_raw_signals.UA` 換瀏覽器等級 UA（PR #177；`repo_health._probe_rss` 共用同一常數、一併治好）。公開訂閱用 feed 以一般 reader 等級 UA 消費屬正常。與 D32 重試互補：D32 治真瞬斷（如 6/16 KR 三源），本修治 UA 級封鎖（7/2 三源的真根因）。
- **教訓**：**「死源」要分層歸因——先換 UA / 換路徑重測，排除「是我們被擋」之後才輪到「源死了 / 瞬斷」**。連續在同一天看到 403 與空 200 混出現、隔天又全活，優先懷疑 WAF 對 client 指紋的間歇封鎖，不是網站抽風。
