...
- **重演**：6｜已硬化：E2 操作註記「不跟隨重導＋不存在路徑對照組」＋ `docs/rankings.md` 逐檔實測狀態＋ `region_reader.md`「已實測可讀法」表（#186 UA → #193 403 → D36 body_fetchable → 08-29 `-L` → 09-03「Firecrawl MCP 未接＝補不動」→ 09-24 KREAM 的 UA 字串）
...
- **第 6 變形（09-24，上一條自己的結論中招）**：第 5 變形寫「逐檔實測後只有 KREAM 真的打不到（HTTP 500）」——實測條件是 curl 預設／Chrome UA。KREAM 擋的是 **UA 字串**：Chrome／curl 全路徑 500，Firefox／Safari／不送 UA 全 200（各 3 次穩定）。09-23 reader 回報「帶 UA 就 200」也不精確（Chrome UA 照樣 500）。→ **教訓：UA 不是「有沒有帶」的二元，是字串本身；換一種非 Chrome UA 是 E2 對照組的一部分。**
...
### 2026-09-07 · 三個 reddit 源默默空了三週——`empty` 永遠升不了級,而「卡住的結論」也會過期
- **發生什麼**:`reddit-sneakers` / `reddit-mfa` / `reddit-techwear` 三源的 `old.reddit.com/.rss` 全部回 **200 但內容是 HTML 封鎖頁、解析 0 則**,佔 31 源的 ~10%,歐美素人落地層整層缺席。實測改回 `www.reddit.com/.rss` → 三源各 **200 XML / 25 則**。而 `sources.yml` 上一條註記寫的正是相反的話:「www 域 .rss 一律 403,改 old.reddit(2026-06-15 實測)」。
- **根因一(結論會過期)**:06-15 那個判斷當時是對的,三個月後 reddit 把封鎖換了方向。**寫進註解的環境結論沒有保鮮期,而它讀起來跟不變的事實一模一樣。** 換源前那條註記被當成既定事實信了三週。
- **根因二(`empty` 結構上升不了級)**:`_probe_rss` 判得完全正確 —— 200 + 0 則 → `empty`。但 `check_source_liveness` 把 `empty` 和 `blocked`(403)、`unreachable` 一起歸進「疑遭阻擋(本探測視角)」,而 `health.yml` 只對「死源候補:」開 issue。**所以一個持續空的源,看門狗每次都看到、每次都不說話。** 這個歸法本身有正當理由(#186 四次誤殺、2026-07-03 heddels/permanent-style 的空 200 確實是 WAF 對 bot UA 餵空),不該直接把 `empty` 改判死源。
- **真正的分界**:`blocked`/`unreachable` 是「誰在看」的陳述;`empty` 是**伺服器完整回答了、而且給的是空的**。差別不在當次結果,在**持續性** —— 連續 N 次探測都 `empty`,就不再是視角問題。要硬化的話是「跨次數的持續性」,不是單次的分類。
- **現在不硬化**(依 D7:新檢查只能由重複出現的教訓硬化而來):本次先修 URL + 記這條。**重演一次就做**——做法是給 liveness 加跨次狀態(比照 kai-notify 的 `fleet_history.json`),連續 ≥3 次 `empty` 才升級成死源候補。
- **重演**：1｜未硬化

### 2026-09-06 · `0/N` 的第三態：`coming soon` 但連預約都還沒開
- **發生什麼**：W36 收斂時，MARKAWARE 9/1 上架的高單價大衣全顯示 `0/3`（バークコート ¥220,000、サックコート ¥143,000、ダブルプリーテッドトラウザーズ ¥88,000）。照字面讀＝「本週最貴的一批貨全部秒殺」。實測 `tags`：帶 `coming soon` 但**沒有** `pre-order`、`available=false`，且另帶 `260917`＝**入荷日 9/17**——**一件都還沒開賣**。同站對照組（シャスールジャケット ¥89,100）`coming soon`＋`pre-order`＋`available=true`。
- **為什麼前兩次的結論擋不住**：W34（Graphpaper）與 W35（marka）都寫成「**預約中**顯示 0 不是完售」。那句話覆蓋了「已開放預約」，沒覆蓋「連預約都還沒開」。**同一窗內三種 `0/N` 同時存在**：PUMA×Graphpaper `0/15` 是真完售、MARKAWARE `0/3` 是未開賣、Morphee `0/1` 是單件庫存賣掉。
- **對策 / 硬化**：`prompts/region_reader.md` 新增「`0/N` 有三種意思」表——判別欄位是 `tags` 有無 `pre-order`，並規定看到 `0/N` 要取**同波次另一個在架品項**當對照組。
- **教訓**：**同一個顯示值的意思會隨站的商業狀態分岔，而庫存數本身不帶那個狀態。** 上兩次的訂正寫的是「這個情況下 0 不是完售」，寫成了**特例**而不是**判別法**——特例只擋得住它自己那一種。
- **重演**：3｜已硬化：`prompts/region_reader.md`「`0/N` 有三種意思」表（W34 Graphpaper → W35 marka → 09-06 MARKAWARE 第三態）
