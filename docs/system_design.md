...
| 排程 | 月報由每月 1 號雲端 agent 自動產；daily 骨架可手動 dispatch `daily-brief.yml` | 視 daily 產線實際跑順後再開每日 schedule |
| 推送 | 無 | Telegram / Notion / Sheets（未拍板，先手動跑順再說） |
| 儲存 | Markdown 檔 | 之後可加 DB / 向量檢索做歷史回顧 |
| 自我檢查 | `validate_repo.py`（格式契約）+ `repo_health.py`（產線新鮮度、文件↔程式碼漂移） | 警告若持續沒人理，再升級成自動修復或排程通知 |
