...
| 分類/簡報 | AI 撰寫由對話中的 agent 做（**不接 repo 內 LLM API**，決策 D5） | — |
| 排程 | **0 支雲端 routine（D16）**：daily / 週挑 / 月報全對話觸發。daily-brief workflow 已於 D30 退役刪檔（與 freeze gate 互斥、signals 被 gitignore，實質無用）；收 RSS 失靈時在本機跑 `generate_daily_brief.py` | — |
