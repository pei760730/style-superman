...
定義 `scripts/score_trends.py` 怎麼打分。目標：在一堆趨勢裡，挑出「還在升、又好拍、又能落地」的那幾個。
...
| **content_potential** 內容潛力 | 好不好拍？ | 沒梗、難視覺化 | 一看就想拍、易示範 |
| **accessibility** 落地度 | 一般人能不能穿/買？ | 天價 / 極端 | 平價易入手易駕馭 |
...
```
heat               0.20
growth             0.30   ← 最重：我們要的是「還在升」
longevity          0.15
content_potential  0.25   ← 次重：拍不出來的趨勢對我們沒用
accessibility      0.10
                   ────
                   1.00
```

權重反映策略偏好：**寧可抓住一個還在升、又好拍的中熱度趨勢，也不追一個已經到頂的大眾品。** 若日後策略轉向（例如要做高端內容），調權重即可，公式不變。
...
| 分數 | 分級 | 行動 |
|------|------|------|
| ≥ 75 | 🔥 主打 push | 優先排進選題、主推 |
| 55–74 | ✅ 採用 use | 進 brief，擇期拍 |
...
`barrel jeans`：heat 4 / growth 5 / longevity 3 / content_potential 4 / accessibility 3
