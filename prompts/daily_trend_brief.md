...
7. 固定填寫 `## 🛒 對我有用 For Me`——**這是行動帳，不是感想區**（2026-06-11 Kai 拍板）：
   ...
   - 「值得入手」每項必含五件事：**單品 ｜ 價格（查不到標 `待查` + 去哪查）｜ 通路 ｜ 時點 ｜ 下一步動作**。
   - 任何進「值得入手」的單品，**同日**依 `prompts/buy_picks.md` 開一張完整挑買卡存
     `reports/buy_picks/YYYY-MM-DD-<slug>.md`，For Me 該項直接連到卡。
...
- 結尾固定附「🛒 對我有用 For Me」（行動帳格式，見任務 7）：值得入手項同日開挑買卡（`prompts/buy_picks.md` → `reports/buy_picks/`），不在 brief 內塞卡片內容。
