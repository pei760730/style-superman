...
每月初由擁有者開口觸發，產出「這個月{地區}男裝什麼品牌 / 單品最紅」的速報（對話觸發、無排程，D16）。每個地區一份、各自獨立執行。
...
3. **收斂排序**：月報主榜固定為 **品牌 Top 5** 與 **單品 Top 5**（單品不限服裝鞋款，配件——包 / 眼鏡 / 飾品 / 錶——同等納入）。
   單品 Top 5 每條附**價位**（查得到寫具體價，查不到標 `待查`）；「本月挑買方向」每條附**價位帶 / 通路**（通路給實測有效連結）；所有來源欄一律 markdown 連結。每條都要附「為什麼這個月紅」+ 來源 + 信心（高/中/低）+ 訊號層級；若主要依據是 L4，不能用「熱賣」「爆紅」「銷量」等硬結論。
...
- 量化硬數據以該地區基準榜為準（歐美：Lyst / StockX；**日本：無量化基準榜**，全靠 L2/L3）；月報與基準榜矛盾時，以基準榜為準並標出差異。日本線無量化基準，主榜不可因此放寬信心標準——撐不起的就標 `待查`。
- 訊號弱的月份就誠實說「本月訊號偏弱」：主榜仍維持 Top 5 欄位，但未達門檻者標 `待查` 或留作觀察，不把弱訊號寫成主榜結論；觀察名單可少列並說明原因。
- **管線薄 → 主動補查，不准空著交差（2026-06-14）**：某地區 / 品類若 RSS 管線可信訊號不足，**先主動 WebSearch / WebFetch 補查**再判斷夠不夠寫；管線沒收到 ≠ 該區沒料。標 `待查` 是「查過仍無可信來源」，**不是「管線沒推給我」的代名詞**——管線是底盤不是答案邊界（見 `CLAUDE.md`）。
